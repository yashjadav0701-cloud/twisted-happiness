...
function switchAdminTab(tab) {
    ...
    if(tab === 'inventory') {
        invBtn.className = "text-white bg-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-4 py-2.5 rounded-full border border-luxury-dark shadow-sm transition-colors";
        ordBtn.className = "text-gray-500 bg-luxury-bg hover:text-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-4 py-2.5 rounded-full border border-luxury-blush shadow-sm relative transition-colors";
        invSec.classList.remove('hidden'); ordSec.classList.add('hidden');
    } else {
        ordBtn.className = "text-white bg-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-4 py-2.5 rounded-full border border-luxury-dark shadow-sm relative transition-colors";
        invBtn.className = "text-gray-500 bg-luxury-bg hover:text-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-4 py-2.5 rounded-full border border-luxury-blush shadow-sm transition-colors";
        ordSec.classList.remove('hidden'); invSec.classList.add('hidden'); switchOrderTab('active'); fetchOrders();
    }
}

function switchOrderTab(tab) {
    const activeBtn = document.getElementById('subtab-active'); const completedBtn = document.getElementById('subtab-completed');
    const activeSec = document.getElementById('admin-active-orders'); const completedSec = document.getElementById('admin-completed-orders'); const title = document.getElementById('order-section-title');
    if(tab === 'active') {
        activeBtn.className = "text-white bg-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-6 py-3 rounded-full border border-luxury-dark shadow-sm transition-colors";
        completedBtn.className = "text-gray-500 bg-luxury-bg hover:text-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-6 py-3 rounded-full border border-luxury-blush shadow-sm transition-colors";
        activeSec.classList.remove('hidden'); completedSec.classList.add('hidden'); title.textContent = "Concierge Desk: Active Workspace";
    } else {
        completedBtn.className = "text-white bg-luxury-rose font-bold text-[9px] uppercase tracking-[0.1em] px-6 py-3 rounded-full border border-luxury-rose shadow-sm transition-colors";
        activeBtn.className = "text-gray-500 bg-luxury-bg hover:text-luxury-dark font-bold text-[9px] uppercase tracking-[0.1em] px-6 py-3 rounded-full border border-luxury-blush shadow-sm transition-colors";
...
function renderActiveOrders() {
    ...
    activeOrders.forEach(order => {
        ...
        if (order.status === 'pending') {
            ...
            statusBadge = `<span class="bg-purple-100 text-purple-700 text-[8px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md border border-purple-200">Awaiting Dispatch</span>`;
            actionButton = `<button type="button" onclick="window.th_pushToShiprocket('${order.id}', event)" class="px-5 py-2.5 rounded-full bg-[#E0F2FE] text-[#1E3A8A] border border-[#BFDBFE] font-bold text-[9px] uppercase tracking-widest hover:bg-[#1E3A8A] hover:text-white transition-colors shadow-sm w-full sm:w-auto"><i class="fas fa-rocket mr-1.5"></i> Push to Shiprocket</button><button type="button" onclick="window.th_markOrderDelivered('${order.id}')" class="px-5 py-2.5 rounded-full bg-luxury-rose text-white border border-luxury-rose font-bold text-[9px] uppercase tracking-widest hover:bg-white hover:text-luxury-rose transition-colors shadow-sm w-full sm:w-auto"><i class="fas fa-dove mr-2"></i> 🕊️ Delivered</button>`;
        }

        container.innerHTML += `<div class="border border-luxury-blush rounded-xl p-5 bg-luxury-bg shadow-sm relative overflow-hidden group hover:border-luxury-rose/50 transition-colors"><div class="absolute top-0 left-0 w-1.5 h-full ${order.status === 'pending' ? 'bg-yellow-400' : (order.status === 'curating' ? 'bg-luxury-gold' : 'bg-luxury-rose')}"></div><div class="flex justify-between items-start mb-4"><div><h4 class="font-logo font-normal text-xl text-luxury-dark mb-0.5">${customerData.name}</h4><span class="text-[8px] font-bold text-gray-400 uppercase tracking-widest"><i class="far fa-clock mr-1"></i> ${date}</span></div>${statusBadge}</div><div class="mb-5">${visualItems}<div class="bg-white border border-luxury-blush p-3 rounded-lg text-gray-500 text-[10px] sm:text-[11px] leading-relaxed font-sans shadow-inner whitespace-pre-wrap">${order.customer_reqs}</div></div><div class="flex flex-col xl:flex-row xl:justify-between xl:items-center border-t border-luxury-blush pt-4 mt-2 gap-4"><div><p class="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Final Invoice Total</p><p class="font-poppins font-extrabold text-luxury-dark text-xl">₹${order.total}</p></div><div class="flex gap-2 w-full xl:w-auto justify-start xl:justify-end flex-wrap">${actionButton}</div></div></div>`;
    });
}
...
async function pushToShiprocket(orderId, event) {
    ...
    try {
        ...
        if(error) throw error;
        if(data && data.success === false) { alert("Shiprocket Error:\n" + data.error); throw new Error(data.error); }
